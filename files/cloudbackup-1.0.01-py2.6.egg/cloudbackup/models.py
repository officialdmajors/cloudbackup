...
# Script Location: /usr/lib/python2.6/site-packages/cloudbackup-1.0.01-py2.6.egg/cloudbackup/models.py
# File Mode: 755
import peewee
from datetime import datetime
from cloudbackup.conf import config, load_config, DATABASE
import hashlib
import json
import sqlite3
import os
...
# Create Database(or check that it exists)
# Here sqlite file system database is used
...
class Backups(SyncedModel):
    ...
    def search(cls, query="", destination="", **kwargs):
        ...
        if kwargs.get("profile"):
            ...
            wheres_based_on_specified_access_key = wheres[:]
            wheres_based_on_specified_access_key.append(Backups.backend_hash << [s3_key, glacier_key])

            # wheres.append(Backups.backend_hash << [s3_key, glacier_key])

        retrieve_all_backups = Backups.select().where(*wheres).order_by(Backups.last_updated.desc())
        retrieve_only_backups_with_known_access_key = Backups.select().where(*wheres_based_on_specified_access_key).order_by(Backups.last_updated.desc())

        return retrieve_all_backups, retrieve_only_backups_with_known_access_key
...
def backup_sqlite(filename):
    """Backup cloudbackup SQLite database to file."""
...
def restore_sqlite(filename):
    """Restore a dump into cloudbackup SQLite database."""
    con = sqlite3.connect(DATABASE)
